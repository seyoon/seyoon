var Day6= function(elDivLeft, elDivRight){
	...
	if(arguments.length==2){
		...
		elBtnPlus.onclick = function(){_onPlusBtnPressed();};
		elDivLeft.appendChild(elBtnPlus);
		
		var _initEvents = function(elTxtBox, elBtnUp, elBtnDown) {
			if(document.addEventListener){
				elTxtBox.addEventListener("blur", function() {oInstance.rangeEvaluation(elTxtBox);}, false);
				elBtnUp.addEventListener("mousedown", function(){_onBtnIncreasePressed(elTxtBox);}, false);
				elBtnUp.addEventListener("mouseout", function(){_finalizeJob(elTxtBox,true);}, false);
				elBtnUp.addEventListener("mouseup", function(){_finalizeJob(elTxtBox,true);}, false);
				elBtnDown.addEventListener("mousedown",function(){_onBtnDecreasePressed(elTxtBox);} , false);
				elBtnDown.addEventListener("mouseout", function(){_finalizeJob(elTxtBox,false);}, false);
				elBtnDown.addEventListener("mouseup", function(){_finalizeJob(elTxtBox,false);}, false);
			}else{
				elTxtBox.attachEvent("onblur", function() {oInstance.rangeEvaluation(elTxtBox);});
				elBtnUp.attachEvent("onmousedown", function(){_onBtnIncreasePressed(elTxtBox);});
				elBtnUp.attachEvent("onmouseout", function(){_finalizeJob(elTxtBox,true);});
				elBtnUp.attachEvent("onmouseup", function(){_finalizeJob(elTxtBox,true);});
				elBtnDown.attachEvent("onmousedown", function(){_onBtnDecreasePressed(elTxtBox);});
				elBtnDown.attachEvent("onmouseout", function(){_finalizeJob(elTxtBox,false);});
				elBtnDown.attachEvent("onmouseup", function(){_finalizeJob(elTxtBox,false);});
			}
		};
		...
		var _onBtnIncreasePressed = function(elArg) {
			...
			oTimeoutJob = setInterval(function(){_autoIncrement(elArg);}, 100);
		};
		...
		var _onBtnDecreasePressed = function(elArg) {
			...
			nInitialValue = Number(elArg.value);
			dStartTime = (new Date()).getTime();
			oTimeoutJob = setInterval(function(){_autoDecrement(elArg);}, 100);
		};
		
		var _autoIncrement = function(elArg) {
			if(((new Date()).getTime() - Number(dStartTime)) < 500)return false;
		...
		var _onPlusBtnPressed = function(){
			...
			elBtnDel.value="-";
			elBtnDel.onclick = function(){_onDelBtnPressed(elBtnDel);};
